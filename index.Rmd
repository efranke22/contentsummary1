...
As mentioned above, linear regression won't work for a GWAS study. As a result, we can implement **marginal regression**. The following example will use data found on [https://www.r-bloggers.com/2017/10/genome-wide-association-studies-in-r/](R-bloggers) which includes 323 individuals (110 Chinese, 105 Indian and 108 Malay) and 2,527,458 SNPs.
...
One useful piece of information not contained in the data mentioned in **minor allele frequency (MAF)**. This represents the frequency of the minor allele in the dataset. We can add this to our SNP information using the `snpstats` package and add MAF to `map`, our dataframe that gives us SNP information.
...
These plots really surprised me. Initially my plan was just to remove all SNPs with a MAF < 1%, figuring that would filter out all SNPs with an `NA` value for allele 1. However, in the plot above on the left we see that while the majority of allele 1 missing SNPs have a MAF < 1%, some have a MAF close to 11%, meaning about `71/646` minor alleles *were* detected yet an `NA` value was still recorded. While I have a very minimal understanding allele reading machines, I would not guess that this `NA` is a machine reading error but rather that something else is going on. Given this information, I decided to compromise and remove all SNPs with a MAF < 3%, as well as all other SNPs with an `NA` value for allele 1. This brings us from 1,651,345 SNPs to 1,293,100 SNPs.
...
One downside to this whole process of removing SNPs with small MAFs is that a major goal of GWAS studies is detect to rare variations on SNPs that could be associated with the trait of interest. Removing SNPs when the MAF is small may result in removing critical data to the study. This is tradeoff emphasizes that makes having more people in your GWAS that may have that rare variation on a SNP is helpful and important in forming meaningful results.
...
Finally, we can analyze all chromosomes. To do this, we simply loop over the SNPs in all chromosomes instead of just those in chromosome 1. Unfortunately, my computer was struggling with this for the time being, but I will update this site shortly with the final plot. 

### Acknowledgemnts 

Thank you to [https://www.r-bloggers.com/2017/10/genome-wide-association-studies-in-r/](R-bloggers) for providing the data for this analysis as well as to my professor Kelsey Grinde for sharing much of the code used in the GWAS above and helping solve many problems I ran into when doing this analysis.